...
def mostrar_tablero(tablero, letras_erroneas):
    ...
    if len(letras_erroneas) > 0:
        print('Letras err�neas:', *letras_erroneas)
        print()
...
def pedir_letra(tablero, letras_erroneas):
    ...
    while not valida:
        ...
        if not valida:
            print('Error, no puede ser un s�mbolo ni un n�mero.')
        else:
            valida = letra not in tablero + letras_erroneas
            if not valida:
                print('Letra repetida, �Por qu� no intenta otra nueva?')
...
def jugar_al_ahorcado():
    try:
        ...
        while len(letras_erroneas) < len(MUNECO):  # pasos 7 y 8
            ...
            if len(letras_erroneas)<len(MUNECO):
...
def jugar_otra_vez():
    return input('Deseas jugar otra vez (introduce s para s� o cualquier otra cosa para no): ')
